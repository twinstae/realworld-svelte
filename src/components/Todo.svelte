<script lang="ts">
  ...
  import type { TodoT } from './Todo.d';
  import { enhance } from '$lib/form';

  export let todo: TodoT;
  export let patch: (res: Response) => Promise<void>;
  export let afterDelete: (todoUid: string)=>void;
</script>

<div class="todo" class:done={todo.done} transition:scale|local={{start:0.7}}>
  ...
  <form action="/todos/{todo.uid}.json?_method=delete" method="post"
        use:enhance={{result: ()=>afterDelete(todo.uid) }}>
